...
import { updateVacancy, deleteVacancy, getVacancyById, getPlatformUserOrgId } from "@/lib/neon-storage";
...
export async function PATCH(
  ...
  // IDOR guard: the target vacancy must belong to the caller's organization.
  const existing = await getVacancyById(numericId);
  if (!existing) return Response.json({ message: "Vacancy not found" }, { status: 404 });
