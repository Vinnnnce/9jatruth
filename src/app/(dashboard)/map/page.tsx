...
import { MapPin, Navigation, Sparkles, Star, Loader2, Mountain, Brain, Search, X, Menu, AlertCircle, RefreshCw } from "lucide-react";
...
export default function GeoMap() {
  ...
  // Debounced search within nearby places
  useEffect(() => {
      ...
      <div className="p-4 md:p-6 max-w-5xl space-y-6">
        <Card>
          <CardContent className="p-6 text-center space-y-3">
            ...
            <p className="text-xs text-muted-foreground">There was an error loading the map. Please try again.</p>
  ...
  const places: NearbyPlace[] = searchResults.length > 0 ? searchResults : (nearbyData?.places || []);

  // Validate and filter places with valid coordinates
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="border-border lg:col-span-2">
          <CardContent className="p-4">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium truncate">{place.name || "Unnamed"}</p>
            ...
            {MAPTILER_KEY ? (
              <div className="rounded-lg overflow-hidden relative" style={{ height: "450px" }} data-testid="google-map">
                {mapError ? (
                  <div className="flex flex-col items-center justify-center h-full bg-muted/30 rounded-lg gap-3">
                    ...
                    <Button variant="outline" size="sm" onClick={() => setMapError(false)} className="gap-1.5">
                      <RefreshCw className="h-3.5 w-3.5" />
                      Retry
                    </Button>
                  </div>
                    ...
                    {selectedPlace && selectedPlace.lat != null && selectedPlace.lng != null && (
                      ...
                      >
                        <div className="p-1 max-w-[200px]">
                          <p className="text-xs font-medium">{selectedPlace.name || "Unnamed Place"}</p>
                          <p className="text-[10px] text-gray-500">{selectedPlace.category || "Unknown category"}</p>
        ...
        {/* Sidebar: mobile - using Sheet */}
        <Sheet open={sidebarOpen} onOpenChange={setSidebarOpen}>
          <SheetContent side="right" className="w-[300px] sm:w-[350px] p-4 overflow-y-auto">
            {sidebarContent}
          </SheetContent>
        </Sheet>
      </div>
        ...
        <Card className="border-amber-500/30 bg-amber-500/5">
          <CardContent className="p-3 flex items-center gap-2">
            ...
            <p className="text-xs text-muted-foreground">
              Could not load nearby places. The Google Maps API key may not be configured.
            </p>
          </CardContent>
        </Card>
      )}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <p className="text-xs font-medium truncate">{place.name || "Unnamed Place"}</p>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-[10px] text-muted-foreground">{place.category || "Unknown"}</span>
                      {place.distance != null && (
