...
const arr = governors.map(([s,g,p,d]) =>
  `INSERT INTO political_candidates (name, party_acronym, office, office_level, election_year, record_type, state, political_background, running_mate, incumbent_since, verification_status, data_confidence, source_urls) VALUES ('${esc(g)}', '${p}', 'governor', 'state', 2027, 'incumbent', '${s}', 'Current Governor of ${s} State. Elected under the ${p} platform.', '${esc(d)}', '2023', 'verified', 70, '["https://en.wikipedia.org/wiki/List_of_current_state_governors_in_Nigeria"]') ON CONFLICT DO NOTHING`
);
fs.writeFileSync("/tmp/gov_sql.json", JSON.stringify(arr));
console.log("wrote", arr.length, "statements");
